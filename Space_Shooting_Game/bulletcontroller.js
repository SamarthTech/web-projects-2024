...
export default class BulletController {
  ...
  collideWith(alien) {
    const bulletThatHitalienIndex = this.bullets.findIndex((bullet) =>
      bullet.collideWith(alien)
    );

    if (bulletThatHitalienIndex >= 0) {               //if bullets hits remove bullet from bulletlist
      this.bullets.splice(bulletThatHitalienIndex, 1);
      return true;
    }
